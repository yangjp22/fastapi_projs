# -*- coding:utf-8 -*-
"""
@Created on: 2022-06-29, Wed, 17:29
@Author: Jinpeng Yang
@Description: Router for api
"""
...
from proj_covid import crud, schemas, models
from proj_covid.database import Base, engine, SessionLocal
...
@router.post('/create_city', response_model=schemas.ReadCity)
def create_city(city: schemas.CreateCity, db: Session = Depends(get_db)):
    db_city = crud.get_city_by_name(db, city.province)
    if db_city:
        raise HTTPException(status_code=400, details='City already registered')
    return crud.create_city(db, city)


@router.get('/get_city/{city}', response_model=schemas.ReadCity)
def get_city(city: str, db: Session = Depends(get_db)):
    db_city = crud.get_city_by_name(db, province=city)
    if not db_city:
        raise HTTPException(status_code=404, detail='City not found')
    return db_city


@router.get('/get_cities/', response_model=list[schemas.ReadCity])
def get_cities(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    cities = crud.get_cities(db, skip=skip, limit=limit)
    return cities


@router.post("/create_data", response_model=schemas.ReadData)
...
@router.get('/get_data', response_model=list[schemas.ReadData])
...
def bg_task(url: HttpUrl, db: Session):
    ...
    if 200 == coronavirus_data.status_code:
        ...
        for city in coronavirus_data.json()["locations"]:
            db_city = crud.get_city_by_name(db=db, province=city["province"])
...
@router.get("/sync_coronavirus_data/jhu")
